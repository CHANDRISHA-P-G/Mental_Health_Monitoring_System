...
function WeeklyReport() {
  ...
  const build7Days = useCallback((serverData) => {
    let week = [];
    for (let i = 6; i >= 0; i--) {
      ...
      week.push(found || {
        ...
        hydration: 0, sleep: 0, exercise: 0, socialBattery: 0
      });
    }
    setData(week);
  }, [referenceDate]);
  ...
  const avgSleep = data.length ? (data.reduce((a, c) => a + (c.sleep || 0), 0) / data.length).toFixed(1) : 0;
  const avgExercise = data.length ? Math.round(data.reduce((a, c) => a + (c.exercise || 0), 0) / data.length) : 0;
  const avgSocial = data.length ? Math.round(data.reduce((a, c) => a + (c.socialBattery || 0), 0) / data.length) : 0;
    ...
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      <Box sx={{ bgcolor: "#F8FAFC", minHeight: "100vh", p: 4, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <Box sx={{ textAlign: "center", mb: 6 }}>
          <Typography variant="h3" fontWeight={900}>Weekly Summary</Typography>

          <Typography sx={{ mb: 2 }}>
            {startDate} — {endDate}
          </Typography>

          <Box sx={{ display: 'inline-flex', bgcolor: '#fff', px: 2, py: 0.5, borderRadius: '50px' }}>
            <DatePicker
              value={referenceDate}
        ...
        <Box sx={{ width: '100%', maxWidth: '1000px' }}>
              ...
              <ChartCard>
                <CardContent>
                  <Stack direction="row" spacing={1.5} mb={2}>
                    ...
                    <Typography fontWeight={800}>Weekly Distribution</Typography>
                  </Stack>
                      ...
                      options={{
                        ...
                        plugins: {
                          legend: {
                            position: "bottom",
                            labels: {
                              usePointStyle: true,     // 🔥 makes it circular
                              pointStyle: "circle",    // 🔥 ensures perfect round
                              boxWidth: 8,
                              boxHeight: 8,
                              padding: 18,
                              font: {
              ...
              <ChartCard>
                <CardContent>
                  <Stack direction="row" spacing={1.5} mb={2}>
                    ...
                    <Typography fontWeight={800}>Weekly Trend</Typography>
                  </Stack>
          ...
          <Grid container spacing={3} justifyContent="center" sx={{ mb: 6 }}>
            {[
              ...
              { label: 'SLEEP', val: `${avgSleep}h`, icon: <BedtimeIcon />, color: '#7E57C2', text: "Avg. Rest", textColor: "#9575CD" },
              { label: 'EXERCISE', val: `${avgExercise}m`, icon: <FitnessCenterIcon />, color: '#26C6DA', text: "Daily Avg", textColor: "#4DD0E1" },
          ...
          <Box sx={{
            ...
            <Typography variant="h6" sx={{ color: '#fff', fontStyle: 'italic' }}>
              "You've maintained consistency this week. Keep going!"
            </Typography>
          </Box>
