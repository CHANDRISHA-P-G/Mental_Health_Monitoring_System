...
function Hydration() {
  ...
  const [pendingValue, setPendingValue] = useState(0); // for submit
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (res.data) {
          setPendingValue(res.data.hydration ?? 0);
        }
      } catch (err) {
        console.log("New day, starting at 0ml");
      } finally {
        setLoading(false);
      }
    };
  ...
  const isExceeded = pendingValue > GOAL;
  const progress = (pendingValue / GOAL) * 100;
    ...
    >
      ...
      >
        <Box
          sx={{
            ...
            height: 110,
            borderRadius: "30px",
            bgcolor: isExceeded ? `${BONUS_COLOR}15` : `${HYDRATION_BLUE}12`,
            display: "inline-flex",
            alignItems: "center",
            justifyContent: "center",
            mb: 3,
        ...
        >
          ...
          <span style={{ fontSize: "1.5rem", color: "#BDC3C7" }}>ml</span>
        </Typography>
        <Box
          sx={{
            width: "100%",
        ...
        >
          <LinearProgress
            ...
            sx={{
              height: 14,
              borderRadius: 10,
              bgcolor: isExceeded ? `${BONUS_COLOR}15` : `${HYDRATION_BLUE}10`,
              "& .MuiLinearProgress-bar": {
                bgcolor: isExceeded ? BONUS_COLOR : HYDRATION_BLUE,
                borderRadius: 10,
              },
            }}
          />
        ...
        <Typography
          ...
          sx={{
            fontWeight: "800",
            color: isExceeded ? BONUS_COLOR : HYDRATION_BLUE,
            letterSpacing: 1.5,
          }}
        >
          {isExceeded
      ...
      >
        ...
        >
          {[
            ...
            <Card
              key={item.label}
              onClick={() => setPendingValue(Math.min(pendingValue + item.amount, 5000))}
              elevation={0}
              sx={{
                display: "flex",
                flexDirection: "column",
            ...
            >
              ...
              >
                <WaterDropIcon sx={{ color: HYDRATION_BLUE, fontSize: 24 }} />
              </Box>
              <Typography
                variant="h6"
                sx={{
      ...
      <Box sx={{ mt: 6, textAlign: "center" }}>
        <Button
          ...
          onClick={() => setPendingValue(Math.max(pendingValue - 250, 0))}
          sx={{
            color: "#BDC3C7",
            fontWeight: "800",
            borderRadius: "12px",
